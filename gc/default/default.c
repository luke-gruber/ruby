...
#if USE_PARALLEL_SWEEP
void wait_for_background_sweeping_to_finish(rb_objspace_t *objspace, bool lock_sweep_thread, bool exit_sweep_thread, const char *from_fn);
# define SET_DONT_GC_ON_PRE(o) wait_for_background_sweeping_to_finish((o), false, false, "dont_gc_on")
#else
...
#else
#define dont_gc_on()          (SET_DONT_GC_ON_PRE(objspace), objspace->flags.dont_gc = 1)
...
#if USE_PARALLEL_SWEEP && PSWEEP_LOCK_STATS
 ...
 *                         or the Ruby thread).
 *   - in_flight:          pages the worker has claimed but the Ruby thread has
 *                         not yet processed. Not stored directly; derived as
 *                         sweep_claim_index - ruby_th_claimed (the worker's
 *                         share of all claims), since the only reader is the
 *                         Ruby thread and done-detection is Ruby-thread-only.
 ...
 *
 * claim_index is a single size_t atomic (sweep_claim_index). Both threads
 * fetch_add by chunk_sz when they claim. The Ruby thread additionally bumps its
 * own ruby_th_claimed (a local, non-atomic field) by the same amount, so the
 * worker's contribution is claim_index - ruby_th_claimed. */

/* Load claim_index (chunks handed out) atomically. */
static inline size_t
sweep_claim_index_load(rb_heap_t *heap)
{
    return (size_t)rbimpl_atomic_value_load(&heap->sweep_claim_index, RBIMPL_ATOMIC_ACQUIRE);
}
...
{
    return (size_t)rbimpl_atomic_value_load(&heap->sweep_claim_index, RBIMPL_ATOMIC_ACQUIRE) == 0 &&
           heap->sweep_claim_overshoot == 0 &&
           heap->in_flight_processed == 0 &&
           heap->ruby_th_claimed == 0;
}
...
#define is_lazy_sweeping(objspace)           (GC_ENABLE_LAZY_SWEEP && (objspace)->flags.during_lazy_sweeping)
...
rb_gc_impl_garbage_object_p(void *objspace_ptr, VALUE ptr)
{
    ...
    return during_lazy_sweep && !marked && page->flags.before_sweep;
}
...
heap_prepare(rb_objspace_t *objspace, rb_heap_t *heap)
{
    ...
    if (heap->total_slots < (gc_params.heap_init_bytes / heap->slot_size) &&
            heap_is_sweep_done(objspace, heap)) {
...
{
#if USE_PARALLEL_SWEEP
    wait_for_background_sweeping_to_finish(objspace, false, false, "objspace_each_objects");
#endif
...
#if USE_PARALLEL_SWEEP
    wait_for_background_sweeping_to_finish(objspace, false, false, "gc_abort");
#endif
...
#if USE_PARALLEL_SWEEP
    wait_for_background_sweeping_to_finish(objspace, false, false, "shutdown_call_finalizer");
#endif
...
#if USE_PARALLEL_SWEEP
void
wait_for_background_sweeping_to_finish(rb_objspace_t *objspace, bool lock_sweep_thread, bool exit_sweep_thread, const char *from_fn)
{
...
gc_post_sweep_page(rb_objspace_t *objspace, rb_heap_t *heap, struct heap_page *sweep_page)
{
    ...
    if (!objspace->flags.immediate_sweep) {
        GC_ASSERT(sweep_page->flags.before_sweep);
    }
#endif
    GC_ASSERT(sweep_page->flags.before_sweep);
        ...
        if (bitset & 1) {
            ...
            switch (BUILTIN_TYPE(vp)) {
              ...
              case T_DATA: {
                ...
                void *data = RTYPEDDATA_GET_DATA(vp);
                void (*dfree)(void *);
...
gc_sweep_start_heap_snapshot(rb_objspace_t *objspace, rb_heap_t *heap)
{
    ...
    struct heap_page *page = NULL;
    ccan_list_for_each(&heap->pages, page, page_node) {
        if (!page->flags.before_sweep) continue; // compacted page
        GC_ASSERT(page->pre_deferred_free_slots == 0);
        rb_darray_append_without_gc(&heap->sweep_pages, page);
    }
}
...
#if USE_PARALLEL_SWEEP
        wait_for_background_sweeping_to_finish(objspace, false, false, "verify_internal_consistency");
#endif
...
#if USE_PARALLEL_SWEEP
    wait_for_background_sweeping_to_finish(objspace, false, false, "gc_start");
#endif
...
gc_exit(rb_objspace_t *objspace, enum gc_enter_event event, unsigned int *lock_lev)
{
    ...
    GC_ASSERT(!is_sweep_thread_p());
#endif
    during_gc = FALSE; // NOTE: sweep thread could still be sweeping even if !during_gc
...
atomic_sub_nounderflow(size_t *var, size_t sub)
{
    ...
    while (1) {
        size_t val = (size_t)rbimpl_atomic_value_load(var, RBIMPL_ATOMIC_RELAXED);
        if (val < sub) sub = val;
...
#if USE_PARALLEL_SWEEP
    wait_for_background_sweeping_to_finish(objspace, false, true, "objspace_free");
#if PSWEEP_LOCK_STATS
    {
        print_lock_stats();
    }
