...
#define _ATOMIC_MARK_IN_BITMAP(bits, page, p)   RUBY_ATOMIC_VALUE_OR((bits)[SLOT_BITMAP_INDEX(page, p)], SLOT_BITMAP_BIT(page, p))
#define _ATOMIC_CLEAR_IN_BITMAP(bits, page, p)  RUBY_ATOMIC_VALUE_AND((bits)[SLOT_BITMAP_INDEX(page, p)], ~SLOT_BITMAP_BIT(page, p))
#define ATOMIC_MARK_IN_BITMAP(bits, p)   _ATOMIC_MARK_IN_BITMAP(bits, GET_HEAP_PAGE(p), p)
#define ATOMIC_CLEAR_IN_BITMAP(bits, p)  _ATOMIC_CLEAR_IN_BITMAP(bits, GET_HEAP_PAGE(p), p)
...
gc_sweep_step(rb_objspace_t *objspace, rb_heap_t *heap)
{
    ...
    if (heap->pre_swept_slots_nodeferred >= sweep_budget) {
        swept_slots = heap->pre_swept_slots_nodeferred - sweep_budget;
    }
