...
static void gc_sweep_pre_finish_heap(rb_objspace_t *objspace, rb_heap_t *heap);
...
static void
gc_sweep_pre_finish_heap(rb_objspace_t *objspace, rb_heap_t *heap)
{
    GC_ASSERT(heap->background_sweep_steps <= ATOMIC_LOAD_RELAXED(heap->foreground_sweep_steps));
        ...
        * GC thrashing (running GC more frequently than required). */
        ...
        if (swept_slots < min_free_slots) {
            ...
            if (is_full_marking(objspace) ||
                ...
                /*fprintf(stderr, "[NOFREE] heap:%ld total_slots:%zu swept_slots:%zu min_free:%zu freed:%zu empty:%zu empty_pages:%zu\n",*/
                        /*heap - heaps, total_slots, swept_slots, min_free_slots, heap->freed_slots, heap->empty_slots, objspace->empty_pages_count);*/
            }
        }
    }
}
...
gc_sweep_finish(rb_objspace_t *objspace)
{
    ...
    for (int i = 0; i < HEAP_COUNT; i++) {
        ...
        /* Finish all heaps now that all empty pages are available for resurrection.
        * During parallel sweep, we defer gc_sweep_finish_heap so that small heaps
        * (which finish early) can resurrect empty pages generated by larger heaps. */
        gc_sweep_finish_heap(objspace, heap);
    }
...
gc_sweep_step(rb_objspace_t *objspace, rb_heap_t *heap)
{
    ...
    if (heap_is_sweep_done(objspace, heap)) {
        ...
        gc_sweep_pre_finish_heap(objspace, heap);
