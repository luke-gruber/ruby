...
typedef struct rb_objspace {
    ...
    struct {
        ...
        size_t major_by_nofree_sweep;
        size_t major_by_nofree_marks;
...
gc_sweep_finish_heap(rb_objspace_t *objspace, rb_heap_t *heap)
{
    ...
    psweep_debug(-1, "[gc] gc_sweep_finish heap:%p (%ld) total_slots:%zu swept_slots:%zu (freed:%zu empty:%zu) min_free:%zu empty_pages:%zu\n",
                 heap, heap - heaps, total_slots, swept_slots, heap->freed_slots, heap->empty_slots, min_free_slots, objspace->empty_pages_count);
    ...
    if (swept_slots < min_free_slots &&
        ...
        * GC thrashing (running GC more frequently than required). */
        ...
        if (swept_slots < min_free_slots) {
            /* Always expand here. The NOFREE flag is deferred to
             * gc_sweep_finish_heap_nofree_check which runs after all heaps
             * are done, when the full tomb is available for resurrection. */
...
/* Deferred version: only runs the NOFREE check, skipping resurrection.
 * Used in gc_sweep_finish after all heaps are done, so empty pages from
 * all heaps are available for resurrection. */
static void
gc_sweep_finish_heap_nofree_check(rb_objspace_t *objspace, rb_heap_t *heap)
{
    size_t total_slots = heap->total_slots;
    size_t swept_slots = heap->freed_slots + heap->empty_slots;
    ...
    if (swept_slots < min_free_slots &&
            ((heap->empty_slots == 0 && total_slots > 0) || heap->freed_slots > heap->empty_slots)) {
        /* Try to resurrect pages from the global tomb (now fully populated). */
        struct heap_page *resurrected_page;
        while (swept_slots < min_free_slots &&
                (resurrected_page = heap_page_resurrect(objspace))) {
            heap_add_page(objspace, heap, resurrected_page, false);
            heap_add_freepage(heap, resurrected_page, "gc_sweep_finish_heap_nofree_check");
            swept_slots += resurrected_page->free_slots;
        }
        ...
        if (swept_slots < min_free_slots) {
            if (is_full_marking(objspace) ||
                ...
                objspace->profile.major_by_nofree_sweep++;
            }
        }
    }
}
...
gc_sweep_finish(rb_objspace_t *objspace)
{
    ...
    for (int i = 0; i < HEAP_COUNT; i++) {
        ...
        /* Re-check NOFREE for heaps that couldn't resurrect enough pages earlier.
         * Now that all heaps are done sweeping, the full tomb is available. */
        gc_sweep_finish_heap_nofree_check(objspace, heap);
    }
    ...
    /* Clamp malloc increase counters to 0 if negative.
     * Background sweep frees can drive these counters negative. We don't want
     * that deficit to carry over as a "free credit" for future allocations,
     * which would delay GC triggering. */
    if (malloc_increase < 0) {
        rbimpl_atomic_size_store((volatile size_t *)&malloc_increase, 0, RBIMPL_ATOMIC_RELAXED);
    }
#if RGENGC_ESTIMATE_OLDMALLOC
    if (objspace->malloc_counters.oldmalloc_increase < 0) {
        rbimpl_atomic_size_store((volatile size_t *)&objspace->malloc_counters.oldmalloc_increase, 0, RBIMPL_ATOMIC_RELAXED);
    }
#endif
        ...
        if (objspace->heap_pages.allocatable_slots == 0 && sweep_slots < min_free_slots) {
            if (!full_marking) {
                if (objspace->profile.count - objspace->rgengc.last_major_gc < RVALUE_OLD_AGE) {
                    ...
                    gc_needs_major_flags |= GPR_FLAG_MAJOR_BY_NOFREE;
                    objspace->profile.major_by_nofree_marks++;
                }
            }
...
enum gc_stat_sym {
    ...
    gc_stat_sym_major_gc_by_nofree,
    gc_stat_sym_major_gc_by_nofree_sweep,
    gc_stat_sym_major_gc_by_nofree_marks,
    gc_stat_sym_major_gc_by_oldgen,
    gc_stat_sym_major_gc_by_shady,
    gc_stat_sym_major_gc_by_force,
    gc_stat_sym_major_gc_by_oldmalloc,
...
#define S(s) gc_stat_symbols[gc_stat_sym_##s] = ID2SYM(rb_intern_const(#s))
        ...
        S(minor_gc_count);
        S(major_gc_count);
        S(major_gc_by_nofree);
        S(major_gc_by_nofree_sweep);
        S(major_gc_by_nofree_marks);
        S(major_gc_by_oldgen);
        S(major_gc_by_shady);
        S(major_gc_by_force);
        S(major_gc_by_oldmalloc);
        S(compact_count);
...
#define SET(name, attr) \
    ...
    SET(major_gc_by_nofree, objspace->profile.major_by_nofree);
    SET(major_gc_by_nofree_sweep, objspace->profile.major_by_nofree_sweep);
    SET(major_gc_by_nofree_marks, objspace->profile.major_by_nofree_marks);
    SET(major_gc_by_oldgen, objspace->profile.major_by_oldgen);
    SET(major_gc_by_shady, objspace->profile.major_by_shady);
    SET(major_gc_by_force, objspace->profile.major_by_force);
    SET(major_gc_by_oldmalloc, objspace->profile.major_by_oldmalloc);
